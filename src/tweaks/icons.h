...
void _action_apply_icon_pack(void *_item)
{
    ...
    if (apply) {
        ...
        sprintf(message_done, "Применение %d иконок", applied);
...
void _menu_temp_action(void *_item)
{
    ...
    if (strcmp(info->path, item->preview_path) != 0) {
        ...
        theme_renderDialog(screen, item->label, "Изменения сохранены", false);
        SDL_BlitSurface(screen, NULL, video, NULL);
        SDL_Flip(video);
        msleep(500);
...
void menu_console_icons(void *_)
{
    if (!_menu_console_icons._created) {
        ...
        strcpy(_menu_console_icons.title, "Иконки консоли");
...
void menu_app_icons(void *_)
{
    if (!_menu_app_icons._created) {
        ...
        strcpy(_menu_app_icons.title, "Иконки приложений");
...
void menu_expert_icons(void *_)
{
    if (!_menu_expert_icons._created) {
        ...
        strcpy(_menu_expert_icons.title, "Экспертные иконки");
...
void menu_icons(void *_)
{
    if (!_menu_icons._created) {
        ...
        strcpy(_menu_icons.title, "Иконки");
        list_addItem(&_menu_icons, (ListItem){.label = "Применить иконки...",
