...
void menu_tools(void *_)
{
    if (!_menu_tools._created) {
        ...
        list_addItemWithInfoNote(&_menu_tools,
                                 (ListItem){
                                     .label = "Создать мульти-диск для игр...",
                                     .action = menu_tools_m3uGenerator},
                                 "Многодисковые ромы требуют создания\n"
                                 "файла списка воспроизведения (.m3u). Это \n"
                                 "позволяет иметь один выбор игры и файл сохранения.");
        ...
        list_addItemWithInfoNote(&_menu_tools,
                                 (ListItem){
                                     .label = "Исправить список Перключателя игр",
