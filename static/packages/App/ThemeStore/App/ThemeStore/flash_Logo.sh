...
if [ -f "./logos/$foldername/image1.jpg" ]; then
	...
	./bin/say "Выберите тему или иконки?"$'\n'\("$foldername"\)$'\n\nA = Тема   B = Отмена  X = Иконки\nSelect = Скрыть меню'
	...
	do
		...
		./bin/say "Загрузка темы "$foldername"..."
		sleep 3
